...
async function initializeMapPlannersWithNewFeatures() {
  try {
    const [esriConfig, Map, MapView, CSVLayer] = await Promise.all([
    ...
    esriConfig.apiKey =
      "AAPKf756cde4d4b34e0c8534982010a238db8rwpvPDYDTBHP2Vvmio5dpW97ZL_dt6Ij_hSt9DNDnZFeMV-GfDMTt6stWK5Sxcm"; // Will change it
    ...
    // }];
        ...
        const hospitals = new CSVLayer({
          url: "https://raw.githubusercontent.com/ashrafayman219/CSVV/main/hospitals_yemen.csv",
          copyright: "data",
          renderer: {
            type: "simple",
            symbol: {
    ...
    view = new MapView({
      center: [44.1756350872286, 15.3706791629834], // longitude, latitude, centered on Yemen
    ...
    view.whenLayerView(hospitals).then(function (layerView) {
      console.log(hospitals, "csvLayer")
      view.goTo(
        {
          target: hospitals.fullExtent,
        },
        {
          duration: 2000,
        }
